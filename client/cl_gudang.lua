...
local function OpenGudangWithPin(kode, pin, lokasi)
    ...
    if not ownedData then
        if Config.Gudang.Framework == "qbcore" then
            QBCore.Functions.Notify('Kode Gudang Atau Pin Salah', 'error', 7500)
        elseif Config.Gudang.Framework == "esx" then
            ESX.ShowNotification('Kode Gudang Atau Pin Salah', 'error', 7500)
        end
        return
    end
    ...
    if ownedData then
        ...
        menulist[#menulist+1] = {
            ...
            onSelect = function()
                ...
                if response then
                    if Config.Gudang.Framework == "qbcore" then
                        QBCore.Functions.Notify('Berhasil Merubah Pin', 'success', 7500)
                    elseif Config.Gudang.Framework == "esx" then
                        ESX.ShowNotification('Berhasil Merubah Pin', 'success', 7500)
                    end
                    return
                end
            end,
        ...
        menulist[#menulist+1] = {
            ...
            onSelect = function()
                ...
                if not hasMoney then
                    if Config.Gudang.Framework == "qbcore" then
                        QBCore.Functions.Notify('Tidak Memiliki Cukup Uang', 'error', 7500)
                    elseif Config.Gudang.Framework == "esx" then
                        ESX.ShowNotification('Tidak Memiliki Cukup Uang', 'error', 7500)
                    end
                    return
                end
